...
checkpoint = torch.load(args.pretrained_local, map_location=torch.device('cpu'),
                        weights_only=True)
model=timm.create_model(f'hf_hub:{args.model_name}',pretrained=False,num_classes=args.num_classes)
...
for idx,(idxs,datas,labels) in enumerate(val_loader.dataset):
