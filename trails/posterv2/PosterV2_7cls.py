...
# from .vit_vae_model import VisionTransformer, PatchEmbed
from .vit_vae_model import VisionTransformer, PatchEmbed
...
class ToChannelFirst(nn.Module):
    def forward(self, x):
        return x.permute(0, 3, 1, 2)  # 根据你的实现调整
...
class pyramid_trans_expr2(nn.Module):
    def __init__(self, img_size=224, num_classes=7, window_size=[28,14,7], num_heads=[2, 4, 8], dims=[64, 128, 256], embed_dim=768,vae=False,layer_embedding=False,votes_sum=0):
        super().__init__()
        self.vae=vae
        ...
        face_landback_checkpoint = torch.load(r'D:\lg\models\mobilefacenet_model_best.pth.tar',
        ...
        self.VIT = VisionTransformer(depth=2, in_c=147,embed_dim=embed_dim,num_classes=num_classes,vae=self.vae)

        self.ir_back = Backbone(50, 0.0, 'ir')
        ir_checkpoint = torch.load(r'D:\lg\models\ir50.pth', map_location=lambda storage, loc: storage)
        ...
        self.embed_k = nn.Sequential(nn.Conv2d(dims[1], 768, kernel_size=3, stride=2, padding=1),
                                    nn.GELU())
        self.embed_v =nn.Sequential(nn.Conv2d(256, 768, kernel_size=1),
                                    nn.GELU())
        self.level_embeds = nn.Parameter(torch.zeros(3, 1, 768))  # 可学习，层位置编码
        self.fusion_norm= nn.LayerNorm(embed_dim)
        self.to_channel_first=ToChannelFirst()
    def forward(self, x):
        ...
        o1, o2, o3 = _to_channel_first(o1), _to_channel_first(o2), self.to_channel_first(o3)
        
        o1, o2, o3 = self.embed_q(o1).flatten(2).transpose(1, 2), self.embed_k(o2).flatten(2).transpose(1, 2), self.embed_v(o3).flatten(2).transpose(1,2)
        #层位置编码
        if self.layer_embedding:
            o1 = o1 + self.level_embeds[0]
            o2 = o2 + self.level_embeds[1]
            o3 = o3 + self.level_embeds[2]
        #cat
        o = torch.cat([o1, o2, o3], dim=1)
        o=self.fusion_norm(o)
        # o = torch.cat([o2, o3], dim=1)
